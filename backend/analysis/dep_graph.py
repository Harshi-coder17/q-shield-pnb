...
class DependencyGraphEngine:
    ...
    #  RISK TAGGING
    # ─────────────────────────────
    def apply_risk_labels(self):
        for node, data in self.G.nodes(data=True):
            if data.get('node_type') == 'algorithm':
                if any(v in node for v in ['RSA', 'ECDSA', 'SHA1']):
                    data['risk'] = 'HIGH'
                elif any(p in node for p in ['ML-DSA', 'Kyber', 'FALCON']):
                    data['risk'] = 'LOW'
                else:
                    data['risk'] = 'MEDIUM'
    ...
    #  ATTACK SIMULATION
    # ─────────────────────────────
    def simulate_attack(self, compromised_algorithm: str) -> dict:
        impacted_certs = []
        impacted_hosts = []

        # Find certificates using this algorithm
        for u, v, d in self.G.edges(data=True):
            if d.get('rel') == 'signed_with' and v == compromised_algorithm:
                impacted_certs.append(u)

        # Find hosts using those certificates
        for u, v, d in self.G.edges(data=True):
            if d.get('rel') == 'uses_certificate' and v in impacted_certs:
                impacted_hosts.append(u)

        return {
            'algorithm': compromised_algorithm,
            'impacted_certificates': list(set(impacted_certs)),
            'impacted_hosts': list(set(impacted_hosts))
        }

    # ─────────────────────────────
    #  BLAST RADIUS
    # ─────────────────────────────
    def blast_radius(self, compromised_algorithm: str) -> dict:
        sim = self.simulate_attack(compromised_algorithm)

        return {
            'algorithm': compromised_algorithm,
            'affected_hosts': len(sim['impacted_hosts']),
            'affected_certificates': len(sim['impacted_certificates']),
            'severity': (
                'CRITICAL' if len(sim['impacted_hosts']) >= 5 else
                'HIGH' if len(sim['impacted_hosts']) >= 2 else
                'MEDIUM'
            )
        }

    # ─────────────────────────────
    # ALGORITHM IMPACT RANKING
    # ─────────────────────────────
    def rank_algorithms_by_risk(self) -> list:
        impact_scores = {}

        for node, data in self.G.nodes(data=True):
            if data.get('node_type') == 'algorithm':
                sim = self.simulate_attack(node)
                impact_scores[node] = len(sim['impacted_hosts'])

        return sorted(
            [{'algorithm': k, 'impact': v} for k, v in impact_scores.items()],
    ...
    def to_json(self) -> dict:
        ...
        #  Apply risk labels before export
        self.apply_risk_labels()
        ...
        return {
            'nodes': nodes,
            'edges': edges,
            'cert_reuse': self.detect_cert_reuse(),

            #  INTELLIGENCE OUTPUT
            'algorithm_risk_ranking': self.rank_algorithms_by_risk()
        }